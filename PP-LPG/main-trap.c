...
void insert_flaw(int node, int state) {
  ...
  for (i = 0; i < numFlaw; i++) {
    if (FlawVect[i].node == node && FlawVect[i].state == state) break;
  }
  if (i == numFlaw) {
...
void create_problem_file(Node *n, char *objFilename, char *initFilename, char *goalFilename) {
  ...
  while(feof(fp) == 0) {
    if (fgets(str, MAX_STR_LEN, fp) != NULL) {fprintf(out, "%s", str);fprintf(out_trap, "%s", str);}
  }
  ...
  fclose(fp);
    ...
    if (n!=NULL){
        ...
        for (i = n->futuregoalcount-1; i >=0; i--) {
           ...
           while(feof(fpt) == 0) {
               if (fgets(str, MAX_STR_LEN, fpt) != NULL) {fprintf(out_trap, "%s", str);}
              }
...
*/
    ...
    fprintf(out_trap, ")))\n");  
	  ...
	  for (i = 0; i < n->numS; i++) {
		  ...
		  } else if (counter % 5 != 0) fprintf(out, ")\n))\n");
	      }
	      fclose(tmp);	  
	  }
	  fprintf(out, "))\n(:metric minimize (+ ");
  ...
  if (n != NULL) { 
    ...
    */  
    ...
    for (i = 0; i < n->numS; i++) {
      ...
      while(feof(tmp) == 0) {
	if (fgets(str, MAX_STR_LEN, tmp) != NULL) fprintf(out, "%s", str);
      }
...
//      NodeVect[i].T = (State *) calloc (MAX_STATE, sizeof(TabuState));
    ...
    if (i == 0) NodeVect[i].numS = NodeVect[i].count = 1;
    else NodeVect[i].numS = NodeVect[i].count = 0;
    NodeVect[i].numT = 0;
    NodeVect[i].T = NULL;
  }
}
...
int countNumActs(char *filename) {
    ...
    while(feof(fp) == 0) {
	if (fgets(str, MAX_STR_LEN, fp) != NULL) i++;
    }
...
void check_graph() {
  ...
  for (i = 0; i < numNode; i++) {
	...
	if (l < NodeVect[i].E[k].numP) {
      ...
      if (count == NodeVect[i].numE && NodeVect[i].numE > 0) { /* c'e' un piano per tutti gli archi */
...
void update_graph(int numNode, int numState) {
  ...
  while(vi < vf) {
    ...
    for (l = 0; l < NodeVect[vi->node].numS; l++)
      if (NodeVect[vi->node].S[l].num == vi->state) break;
	...
	if (NodeVect[vi->node].E[i].P[j].state == vi->state) break;
      }
	...
	for (k = 0; k < NodeVect[NodeVect[vi->node].E[i].to_node].numS; k++) {
	  for (l = 0; l < NodeVect[NodeVect[vi->node].E[i].to_node].S[k].numSource; l++) {
	    if (NodeVect[NodeVect[vi->node].E[i].to_node].S[k].sourceNode[l] == vi->node && NodeVect[NodeVect[vi->node].E[i].to_node].S[k].sourceState[l] == vi->state) {  /* remove the source */
	      if (DEBUG2) {
...
void print_flaws() {
  ...
  printf("\n\nList of flaw:\n");
  for (ind = 0; ind < numFlaw; ind++) printf("NODE: %d STATE %d\n", FlawVect[ind].node, FlawVect[ind].state);
}
...
for (int i=0;i<MAX_STEPS;i++){
  ...
  if (x1->numE ==0 ) break; //no outgoing edges
...
// will be dropped 
  for (indRestart = 0; indRestart < MAX_NUM_RESTART; indRestart++) {
	...
	for (l = 0; l < e->numP; l++) {
	  if (e->P[l].state == flawState) break;
	}

	if (l == e->numP) { /* there exists NO pair p labelling e such that State(p) = s */
...
//		(e->numP > 1 || (e->numP == 1 && x1->num != 0) )) { 
...
#endif
#else
	  if (x2->numS > 0) pref_flag = TRUE;
	  else pref_flag = FALSE;
#ifndef __HPLANP__
	  ...
	  if (e->numP == 0) { // || (e->numP == 1 && x1->num == 0)) {
	      ...
	      if (selectPlanToAdapt == RANDOM) i = rand() % e->numP;
	      else if (selectPlanToAdapt == BEST)
		  for(i = 0; i < e->numP; i++) {
		      if (e->P[i].state == bestState) break;
		  }
...
//	    exit(0);
	  ...
	  if (m == x2->numS) { /* EndState does not label x2 */
	...
	if (x1->num == 0 && flawState == 0)
	  ...
	  if (l < x1->numS) { 
		...
		if (NodeVect[sn].E[i].P[j].state == ss) break;
	      }
