...
public:
	...
	DFS_Plus( const Search_Model& search_problem, PartialStateCollection* psc ) : m_problem( search_problem ), m_pruned_sum_B_count(0), m_sum_B_count(0), m_max_B_count(0), m_iw_calls(0), m_expanded(0), m_generated(0), m_goal_agenda(NULL), m_max_bound( std::numeric_limits<unsigned>::max() ), m_backtracks(0), m_root(NULL), m_iw_engine( search_problem, psc ), m_psc_traps(psc), m_h2_traps_count(0), m_psc_traps_count(0), m_reachable_traps_count(0) {	   	
		m_goal_agenda = NULL;
	...
	bool            dfs_search( State* init, std::vector<Action_Idx>& plan, float& cost, Fluent_Vec goals_achieved ){
		...
		do{
			...
			m_psc_traps_count += m_iw_engine.psc_traps_count();
