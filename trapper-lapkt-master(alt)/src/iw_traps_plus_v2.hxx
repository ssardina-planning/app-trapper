...
public:
    ...
    virtual bool check_novelty_1( unsigned cand_idx, PartialStateCollection* head ) {
        if( ! m_novelty_table_vec[head->trap_size + 1].isset(cand_idx) )
            return true;
    ...
    void process( PartialStateCollection* head) {
		...
		else{// Mark candidate as forbidden for any trap
		    for(unsigned i = 0; i < m_novelty_table_vec.size(); i++)
			m_novelty_table_vec[i].set(k);
		}
    ...
    virtual bool find_traps() {
	    ...
	    //If has been marked as escape, do not consider
	    if( m_novelty_table_vec[1].isset(i) ) continue;
